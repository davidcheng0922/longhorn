...
- Ublk volume / Sharding are not supported
...
- Live upgrade is supported only when upgrading from Longhorn `v1.12.2` or later
...
- Retry failed nodes after all nodes receive an initial upgrade attempt
...
2. **InstanceManagerUpgradeControlController**: Orchestrates cluster-wide rolling upgrades. Enforces one-node-at-a-time execution, records terminal node failures, and manages orphaned upgrades.

### Core Mechanism

Live upgrade coordinates existing V2 data-plane behavior. It does not introduce a new data-plane component, transport, or I/O redirection mechanism.

1. **Engine switchover**: The Volume and EngineFrontend controllers use the existing NVMe/TCP ANA switchover to move the engine target to a temporary node. The host-side initiator and attached block-device path stay on the source node.
2. **In-place image upgrade**: After engines and planned source-node replicas are moved out of the upgrading instance manager, the InstanceManager controller patches the existing pod's image. Kubernetes restarts the container in place.
3. **Normal replica recovery**: Engines switch back after the upgraded instance manager is ready. The regular Volume controller then reuses or rebuilds detached replicas until the volumes are healthy.

The IMU controller only orders these existing operations and waits for their readiness and health conditions.
...
- **Bounded Retries**: After every pending node receives an initial attempt, failed nodes are retried up to five times. A retry resumes the same IMU and its persisted relocation plan.
...
- **Failed**: The latest upgrade attempt failed. The node is retried after all pending nodes receive an initial attempt, up to the retry limit.

**Note:** Nodes whose active IM already runs the target image are excluded from candidate selection. Existing terminal status entries are retained as upgrade history.
...
6. On IMU failure: marks the node `Failed` and continues with the next pending node
7. After no pending nodes remain: retries failed nodes up to five times, one at a time
...
|-------|------|-------------|
| `NodeID` | string | Source node where the instance manager is running. Immutable after creation. |
| `TargetImage` | string | Desired instance manager image after upgrade. Immutable after creation. |
...
| `AbortRequested` | bool | Controller-managed flag indicating that an abort has been triggered due to timeout, target image change, or no temporary node |
| `AbortReason` | string | Explains why abort was requested: "timeout", "target-image-changed", or "no-temporary-node" |
...
| `RetryCount` | int | Number of retries after the initial node upgrade attempt. Failed nodes are retried up to five times after all pending nodes receive an initial attempt. |
...
- Engines are relocated only to nodes that can safely host the volume, meaning the node has an active replica currently in `ReplicaModeRW`, a running v2 AllInOne instance manager, and a schedulable block disk.
...
- Among pending nodes, the controller prioritizes nodes that host v2 engines. It then selects replica-only nodes. Within each group, it picks the lexicographically smallest node ID to keep the order deterministic.
...
- If NO alternative temp node exists: requests an abort and restores engines to the source node when the source IM is ready
...
For restoration, a source node with configured block disks requires at least one schedulable block disk. A source node with no block disks can still restore an engine that uses remote replica backends.
...
**Behavior**:
- A target image change starts a new upgrade cycle. Nodes are re-evaluated against the new image without storing the target image in `NodeUpgradeInfo`.
- **For an active node**: `processCurrentNode()` detects the mismatch and sets `IMU.Status.AbortRequested = true` and `AbortReason = "target-image-changed"`. The IMU restores engines before transitioning to `Failed`.
- **For terminal nodes**: if the active source IM is not on the new target image, IMUC resets the node to `Pending` for the new cycle. This does not require the previous IMU to still exist. Nodes already running the new target image are excluded while their terminal history is retained.
- **Result**: Every node that has not converged to the new image is considered again in the new cycle.
...
- **On timeout detection**:
  ...
  - IMUC marks the node `Failed`, continues with other pending nodes, then retries it after the initial pass if retry budget remains
- **Source node deletion exception**: If the source node is deleted while restoring engines, the IMU fails immediately and abandons restoration.
...
- Marks the orphaned IMU `Failed` without aborting or restoring engines
- Marks the corresponding IMUC node `Failed` and continues the current rolling upgrade
- After the initial pass, retries the same IMU from its persisted relocation and replica-detachment plan if retry budget remains
...
#### 9. Source Node Deleted During Restoration

**Scenario**: The source node is deleted after engines have been relocated and before they can be restored.

**Behavior**:
- The IMU transitions directly to `Failed` and clears its relocation and replica-detachment plans.
- Engines remain on healthy temporary nodes; Longhorn does not wait indefinitely for the deleted source node.
- IMUC records the node as `Failed` and continues with other pending nodes.
- The node is not retried because the deleted source node cannot host an engine.

**Rationale**: A deleted node cannot host a restored engine. Preserving the temporary engine is safer than forcing a restore that cannot complete.

#### 10. IMU Deletion and Uninstall

**Normal IMU deletion**:
- Deleting an active IMU requests graceful engine restoration before its finalizer is removed.
- Deleting an IMU already in `Failed` removes its finalizer without another restoration attempt.

**Uninstall**:
- The uninstall controller disables automatic V2 instance manager upgrade before deleting upgrade resources.
- It then removes IMU finalizers and deletes IMUs and the IMUC directly, deliberately bypassing graceful restoration while the manager admission webhook remains available.
...
**Failure isolation and retry**: A failed node does not block remaining initial attempts. After they finish, it is retried up to five times from its persisted IMU plan
...
**Orphan handling**: Orphaned IMUs are failed without restoration so they cannot create a second active upgrade for the node
...
The v2 live upgrade capability is introduced as a new CRD-based feature. To avoid entering the feature from an unsupported state, Longhorn rejects a system upgrade from a version earlier than `v1.12.2`.
